...
func main() {
	...
	sqlConnect := createSqlConnectForPgSql()
	...
	{
		// insert some rows
		sql := "INSERT INTO tbl_demo (id, name) VALUES ($1, $2)"
		n := 100
	...
	{
		// query single row
		sql := "SELECT name,id FROM tbl_demo WHERE id=$1"
	...
	{
		// query multiple rows
		sql := "SELECT name AS MyName,id AS myid FROM tbl_demo WHERE id>=$1 LIMIT 4"
...

	{
		...
		sql := "SELECT id,name FROM tbl_demo WHERE id>=$1 LIMIT 4"
