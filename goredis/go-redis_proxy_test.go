...
var _setupTestRedisProxy _testSetupOrTeardownFunc = func(t *testing.T, testName string) {
	hosts := []string{"localhost:7000", "localhost:7001", "localhost:7002", "localhost:7003", "localhost:7004", "localhost:7005", "localhost:7006", "localhost:7007"}
	for _, host := range hosts {
		conn := _newGoRedisConnectForRedisClient(t, testName, host, "")
		client := conn.GetClient(0)
		client.SlaveOf(context.Background(), "no", "one")
		client.ConfigSet(context.TODO(), "appendonly", "no")
		client.ConfigSet(context.TODO(), "save", "")
		client.FlushAll(context.TODO())
	}

	// localhost:7005 --> localhost:7000
	// localhost:7004 --> localhost:7001
	// localhost:7003 --> localhost:7002
	_newGoRedisConnectForRedisClient(t, testName, "localhost:7005", "").GetClient(0).SlaveOf(context.Background(), "localhost", "7000")
	_newGoRedisConnectForRedisClient(t, testName, "localhost:7004", "").GetClient(0).SlaveOf(context.Background(), "localhost", "7001")
	_newGoRedisConnectForRedisClient(t, testName, "localhost:7003", "").GetClient(0).SlaveOf(context.Background(), "localhost", "7002")
...
func TestRedisProxy_BitGetBit(t *testing.T) {
...
func TestRedisProxy_BitSetBit(t *testing.T) {
...
func TestRedisProxy_Cluster_ReadOnly(t *testing.T) {
	testName := "TestRedisProxy_Cluster_ReadOnly"
	teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
	defer teardownTest(t)
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
...
func TestRedisProxy_Cluster_ReadWrite(t *testing.T) {
	testName := "TestRedisProxy_Cluster_ReadWrite"
	teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
	defer teardownTest(t)
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
...
func TestRedisProxy_Generic_RestoreReplace(t *testing.T) {
	testName := "TestRedisProxy_Generic_RestoreReplace"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.Set(context.TODO(), key, "value", 0)
			dumpResult := c.Dump(context.TODO(), key)
			c.RestoreReplace(context.TODO(), key+"new", 10*time.Second, dumpResult.Val())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "restore", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_Generic_ScanType(t *testing.T) {
	testName := "TestRedisProxy_Generic_ScanType"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ScanType(context.TODO(), 1234, key+"*", 2, "STRING")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "scan", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_Generic_SortRO(t *testing.T) {
	testName := "TestRedisProxy_Generic_SortRO"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.SortRO(context.TODO(), key, &redis.Sort{})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "sort_ro", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_GeoRadiusStore(t *testing.T) {
	testName := "TestRedisProxy_GeoRadiusStore"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.GeoRadiusStore(context.TODO(), key, 13.361389, 38.115556, &redis.GeoRadiusQuery{Radius: 12.34, Store: key + "new"})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "geo_radius", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_GeoRadiusByMemberStore(t *testing.T) {
	testName := "TestRedisProxy_GeoRadiusByMemberStore"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.GeoRadiusByMemberStore(context.TODO(), key, "member1", &redis.GeoRadiusQuery{Radius: 12.34, Store: key + "new"})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "geo_radius", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_HMSet(t *testing.T) {
	testName := "TestRedisProxy_HMSet"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.HMSet(context.TODO(), key, "field1", "value1", "field2", "value2")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "hmset", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_List_BRPopLPush(t *testing.T) {
	testName := "TestRedisProxy_List_BRPopLPush"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "{key}"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.BRPopLPush(context.TODO(), key+"src", key+"dest", 1*time.Second)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "brpop_lpush", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_List_LMPop(t *testing.T) {
	testName := "TestRedisProxy_List_LMPop"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "{key}"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.LMPop(context.TODO(), "LEFT", 1, key+"1", key+"2", key+"3")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "lmpop", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_List_RPopCount(t *testing.T) {
	testName := "TestRedisProxy_List_RPopCount"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.RPopCount(context.TODO(), key, 1)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "rpop", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}

func TestRedisProxy_List_RPopLPush(t *testing.T) {
	testName := "TestRedisProxy_List_RPopLPush"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "{key}"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.RPopLPush(context.TODO(), key+"src", key+"dest")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "rpop_lpush", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_PubSub_PubSubShardChannels(t *testing.T) {
	testName := "TestRedisProxy_PubSub_PubSubShardChannels"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.PubSubShardChannels(context.TODO(), "pattern")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "pubsub_shard_channels", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}

func TestRedisProxy_PubSub_PubSubShardNumSub(t *testing.T) {
	testName := "TestRedisProxy_PubSub_PubSubShardNumSub"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.PubSubShardNumSub(context.TODO(), "channel1", "channel2", "channel3")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "pubsub_shard_num_sub", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_PubSub_Publish(t *testing.T) {
	...
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			...
			key := "{channel}"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.Publish(context.TODO(), key, "message")
...
func TestRedisProxy_PubSub_Subscribe(t *testing.T) {
	...
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			...
			case "FAILOVER":
				c := rc.GetFailoverClientProxy(0)
				pubsub = c.Subscribe(context.TODO(), key+"1", key+"2", key+"3")
			case "CLUSTER":
				c := rc.GetClusterClientProxy()
				pubsub = c.Subscribe(context.TODO(), key+"1", key+"2", key+"3")
			default:
...
func TestRedisProxy_PubSub_PSubscribe(t *testing.T) {
	...
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			...
			case "FAILOVER":
				c := rc.GetFailoverClientProxy(0)
				pubsub = c.PSubscribe(context.TODO(), key+"1", key+"2", key+"3")
			case "CLUSTER":
				c := rc.GetClusterClientProxy()
				pubsub = c.PSubscribe(context.TODO(), key+"1", key+"2", key+"3")
			default:
...
func TestRedisProxy_PubSub_SPublish(t *testing.T) {
	testName := "TestRedisProxy_PubSub_SPublish"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.SPublish(context.TODO(), key, "message")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "spublish", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_PubSub_SSubscribe(t *testing.T) {
	testName := "TestRedisProxy_PubSub_SSubscribe"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			switch tc {
			case "FAILOVER":
				client := c.(*RedisFailoverClientProxy)
				pubsub = client.SSubscribe(context.TODO(), key+"1", key+"2", key+"3")
			case "CLUSTER":
				client := c.(*RedisClusterClientProxy)
				pubsub = client.SSubscribe(context.TODO(), key+"1", key+"2", key+"3")
			default:
				client := c.(*RedisClientProxy)
				pubsub = client.SSubscribe(context.TODO(), key+"1", key+"2", key+"3")
			}
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "ssubscribe", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_Script_EvalRO(t *testing.T) {
	testName := "TestRedisProxy_Script_EvalRO"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.EvalRO(context.TODO(), "return ARGV[1]", nil, "hello")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "eval_ro", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_Script_EvalShaRO(t *testing.T) {
	testName := "TestRedisProxy_Script_EvalShaRO"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			scriptSha := c.ScriptLoad(context.TODO(), "return ARGV[1]")
			c.EvalShaRO(context.TODO(), scriptSha.Val(), nil, "hello")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "eval_sha_ro", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FCall(t *testing.T) {
	testName := "TestRedisProxy_Script_FCall"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			funcCode := "#!lua name=mylib\nlocal function myfunc(keys, args)\nreturn args[1]\nend\nredis.register_function{function_name='myfunc', callback=myfunc, flags={ 'no-writes' }}"
			c.FunctionLoad(context.TODO(), funcCode)
			c.FunctionLoadReplace(context.TODO(), funcCode)
			c.FCall(context.TODO(), "myfunc", []string{}, "hello")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "fcall", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FCallRO(t *testing.T) {
	testName := "TestRedisProxy_Script_FCallRO"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionLoad(context.TODO(), funcCode)
			c.FunctionLoadReplace(context.TODO(), funcCode)
			c.FCallRO(context.TODO(), "myfunc", []string{}, "hello")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "fcall_ro", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionDelete(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionDelete"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionDelete(context.TODO(), "mylib")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_delete", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionDumpFlushRestore(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionDumpFlushRestore"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionLoadReplace(context.TODO(), funcCode)
			time.Sleep(1500 * time.Millisecond)
			dumpResult := c.FunctionDump(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_dump", nil, prom.MetricsCatAll, prom.MetricsCatOther)
			c.FunctionFlush(context.TODO())
			time.Sleep(1500 * time.Millisecond)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_flush", nil, prom.MetricsCatAll, prom.MetricsCatOther)
			time.Sleep(1500 * time.Millisecond)
			c.FunctionRestore(context.TODO(), dumpResult.Val())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_restore", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionFlushAsync(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionFlushAsync"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionFlushAsync(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_flush", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionKill(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionKill"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			if tc != "SINGLE" {
				t.Skipf("%s skipped: ignore test on cluster or sentinel", testName+"/"+tc)
			}
			...
			c.FunctionKill(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_kill", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionList(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionList"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionList(context.TODO(), redis.FunctionListQuery{LibraryNamePattern: "mylib"})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_list", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Script_FunctionStats(t *testing.T) {
	testName := "TestRedisProxy_Script_FunctionStats"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.FunctionStats(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "function_stats", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_Server_BgRewriteAOF(t *testing.T) {
	testName := "TestRedisProxy_Server_BgRewriteAOF"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.BgRewriteAOF(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "bg_rewrite_aof", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_BgSave(t *testing.T) {
	testName := "TestRedisProxy_Server_BgSave"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.BgSave(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "bg_save", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_Command(t *testing.T) {
	testName := "TestRedisProxy_Server_Command"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.Command(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "command", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_CommandGetKeys(t *testing.T) {
	testName := "TestRedisProxy_Server_CommandGetKeys"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.CommandGetKeys(context.TODO(), "MSET", "a", "b", "c", "d", "e", "f")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "command_get_keys", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_CommandGetKeysAndFlags(t *testing.T) {
	testName := "TestRedisProxy_Server_CommandGetKeysAndFlags"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.CommandGetKeysAndFlags(context.TODO(), "MSET", "a", "b", "c", "d", "e", "f")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "command_get_keys_and_flags", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_CommandList(t *testing.T) {
	testName := "TestRedisProxy_Server_CommandList"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.CommandList(context.TODO(), nil)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "command_list", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_ConfigGet(t *testing.T) {
	testName := "TestRedisProxy_Server_ConfigGet"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ConfigGet(context.TODO(), "save")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "config_get", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_ConfigResetStat(t *testing.T) {
	testName := "TestRedisProxy_Server_ConfigResetStat"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ConfigResetStat(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "config_reset_stat", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_ConfigRewrite(t *testing.T) {
	testName := "TestRedisProxy_Server_ConfigRewrite"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ConfigRewrite(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "config_rewrite", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_ConfigSet(t *testing.T) {
	testName := "TestRedisProxy_Server_ConfigSet"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ConfigSet(context.TODO(), "save", "")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "config_set", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_Server_LastSave(t *testing.T) {
	testName := "TestRedisProxy_Server_LastSave"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.LastSave(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "last_save", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_MemoryUsage(t *testing.T) {
	testName := "TestRedisProxy_Server_MemoryUsage"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.Set(context.TODO(), key, "value", 0)
			c.MemoryUsage(context.TODO(), key, 1)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "memory_usage", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_Save(t *testing.T) {
	testName := "TestRedisProxy_Server_Save"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.Save(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "save", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_SlowLogGet(t *testing.T) {
	testName := "TestRedisProxy_Server_SlowLogGet"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.SlowLogGet(context.TODO(), 1)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "slow_log_get", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}

func TestRedisProxy_Server_Time(t *testing.T) {
	testName := "TestRedisProxy_Server_Time"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.Time(context.TODO())
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "time", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_Set_SInterCard(t *testing.T) {
	testName := "TestRedisProxy_Set_SInterCard"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.SInterCard(context.TODO(), 1, key+"1", key+"2")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "sinter_card", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_SSet_BZMPop(t *testing.T) {
	...
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			...
			c.BZMPop(context.TODO(), 10*time.Second, "MAX", 1, key+"1", key+"2", key+"3")
...
func TestRedisProxy_SSet_ZAdd(t *testing.T) {
...
func TestRedisProxy_SSet_ZAddGT(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZAddGT"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ZAddGT(context.TODO(), key, redis.Z{Member: "member1", Score: 1.23}, redis.Z{Member: 2, Score: 2.34}, redis.Z{Member: true, Score: 3.45})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zadd", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}

func TestRedisProxy_SSet_ZAddLT(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZAddLT"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ZAddLT(context.TODO(), key, redis.Z{Member: "member1", Score: 1.23}, redis.Z{Member: 2, Score: 2.34}, redis.Z{Member: true, Score: 3.45})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zadd", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}

func TestRedisProxy_SSet_ZAddNX(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZAddNX"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ZAddNX(context.TODO(), key, redis.Z{Member: "member1", Score: 1.23}, redis.Z{Member: 2, Score: 2.34}, redis.Z{Member: true, Score: 3.45})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zadd", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}

func TestRedisProxy_SSet_ZAddXX(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZAddXX"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ZAddXX(context.TODO(), key, redis.Z{Member: "member1", Score: 1.23}, redis.Z{Member: 2, Score: 2.34}, redis.Z{Member: true, Score: 3.45})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zadd", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}

func TestRedisProxy_SSet_ZAddArgsIncr(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZAddArgsIncr"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.ZAddArgsIncr(context.TODO(), key, redis.ZAddArgs{
				Members: []redis.Z{redis.Z{Member: "member1", Score: 1.23}},
			})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zadd", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
...
func TestRedisProxy_SSet_ZInterCard(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZInterCard"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.ZInterCard(context.TODO(), 1, key+"1", key+"2", key+"3")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zinter_card", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_SSet_ZMPop(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZMPop"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.ZAdd(context.TODO(), key+"1", redis.Z{Member: "member1", Score: 1.23})
			c.ZAdd(context.TODO(), key+"2", redis.Z{Member: 2, Score: 2.34})
			c.ZAdd(context.TODO(), key+"3", redis.Z{Member: true, Score: 3.45})
			c.ZMPop(context.TODO(), "MIN", 1, key+"1", key+"2", key+"3")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zmpop", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
...
func TestRedisProxy_SSet_ZRandMemberWithScores(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZRandMemberWithScores"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.ZRandMemberWithScores(context.TODO(), key, 1)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zrand_member", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_SSet_ZRankWithScore(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZRankWithScore"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			rc, c := _getRedisConnectAndCmdable(tc, key)
			ver := _getRedisVersion(c)
			if ver.Compare(v7_2_0) < 0 {
				t.Skipf("%s skipped: Redis version %s does support the specified command, need version %s", testName+"/"+tc, ver, v7_2_0)
			}
			c.ZRankWithScore(context.TODO(), key, "member")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zrank", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_SSet_ZRevRankWithScore(t *testing.T) {
	testName := "TestRedisProxy_SSet_ZRevRankWithScore"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.ZRevRankWithScore(context.TODO(), key, "member")
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "zrev_rank", []error{redis.Nil}, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_Stream_XClaim(t *testing.T) {
	testName := "TestRedisProxy_Stream_XClaim"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.XGroupCreateMkStream(context.TODO(), key, "group", "0-0")
			c.XClaim(context.TODO(), &redis.XClaimArgs{Stream: key, Group: "group", Consumer: "consumer", MinIdle: 3600 * time.Second, Messages: []string{"0-0"}})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "xauto_claim", nil, prom.MetricsCatAll, prom.MetricsCatOther)
		})
	}
}
...
func TestRedisProxy_String_LCS(t *testing.T) {
	testName := "TestRedisProxy_String_LCS"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			c.LCS(context.TODO(), &redis.LCSQuery{
				Key1: key + "1",
				Key2: key + "2",
			})
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "lcs", nil, prom.MetricsCatAll, prom.MetricsCatDQL)
		})
	}
}
...
func TestRedisProxy_String_SetXX(t *testing.T) {
	testName := "TestRedisProxy_String_SetXX"
	for _, tc := range _testList {
		t.Run(tc, func(t *testing.T) {
			teardownTest := setupTest(t, testName, _setupTestRedisProxy, _teardownTestRedisProxy)
			defer teardownTest(t)
			...
			key := "key"
			rc, c := _getRedisConnectAndCmdable(tc, key)
			c.SetXX(context.TODO(), key, "value", 10*time.Second)
			_rcVerifyLastCommand(func(msg string) { t.Fatalf(msg) }, testName+"/"+tc, rc, "set", nil, prom.MetricsCatAll, prom.MetricsCatDML)
		})
	}
}
