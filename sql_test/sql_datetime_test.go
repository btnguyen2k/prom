		...
		t.Run(dbtype, func(t *testing.T) {
			...
			for i, row := range rows {
				...
				}
				{
					...
					if sqlc.GetDbFlavor() != prom_sql.FlavorOracle && sqlc.GetDbFlavor() != prom_sql.FlavorSqlite {
						v = _changeLoc(v, e.Location())
					}
					if estr, vstr := e.In(LOC2).Format(layout), v.In(LOC2).Format(layout); !ok || vstr != estr {
						t.Fatalf("%s failed: [%s]\nexpected %#v/%#v/%#v(%T)\nbut received %#v/%#v/%#v(%T) (Ok: %#v)", testName,
...
func TestSql_DataTypeNull(t *testing.T) {
	...
	colTypesMap := map[prom_sql.DbFlavor][]string{
		prom_sql.FlavorCosmosDb: nil,
		prom_sql.FlavorMsSql:    {"NVARCHAR(8)", "INT", "DOUBLE PRECISION", "NVARCHAR(64)", "MONEY", "DATE", "TIME", "DATETIME2", "BIGINT"},
		prom_sql.FlavorMySql:    {"VARCHAR(8)", "INT", "DOUBLE", "VARCHAR(64)", "DECIMAL(36,2)", "DATE", "TIME", "DATETIME", "BIGINT"},
		prom_sql.FlavorOracle:   {"NVARCHAR2(8)", "INT", "DOUBLE PRECISION", "NVARCHAR2(64)", "NUMERIC(36,2)", "DATE", "TIMESTAMP(0)", "TIMESTAMP(0)", "INTERVAL DAY TO SECOND"},
		prom_sql.FlavorPgSql:    {"VARCHAR(8)", "INT", "DOUBLE PRECISION", "VARCHAR(64)", "NUMERIC(36,2)", "DATE", "TIME(0)", "TIMESTAMP(0)", "BIGINT"},
		prom_sql.FlavorSqlite:   {"VARCHAR(8)", "INT", "DOUBLE", "VARCHAR(64)", "DECIMAL(36,2)", "DATE", "TIME", "DATETIME", "BIGINT"},
	}
	type Row struct {
		id           string
		dataInt      *int64
		...
		t.Run(dbtype, func(t *testing.T) {
			...
			for i, row := range rows {
				...
				}
				{
					...
					} else if row[f] != (*int64)(nil) && (sqlc.GetDbFlavor() == prom_sql.FlavorCosmosDb && row[f] != nil) {
				...
				}
				{
					...
					} else if row[f] != (*float64)(nil) && (sqlc.GetDbFlavor() == prom_sql.FlavorCosmosDb && row[f] != nil) {
						t.Fatalf("%s failed: [%s] expected nil but received %#v(%T)", testName, row["id"].(string)+"/"+f, row[f], row[f])
					}
				}
				{
					...
					} else if row[f] != (*string)(nil) && (sqlc.GetDbFlavor() == prom_sql.FlavorCosmosDb && row[f] != nil) {
						t.Fatalf("%s failed: [%s] expected nil but received %#v(%T)", testName, row["id"].(string)+"/"+f, row[f], row[f])
					}
				}
				{
				...
				}
				{
					...
					} else if row[f] != (*time.Time)(nil) && ((sqlc.GetDbFlavor() == prom_sql.FlavorCosmosDb) && row[f] != nil) {
						t.Fatalf("%s failed: [%s] expected nil but received %#v(%T)", testName, row["id"].(string)+"/"+f, row[f], row[f])
					}
				}
				{
...
func TestSql_Timezone(t *testing.T) {
	...
	colTypesMap := map[prom_sql.DbFlavor][]string{
		prom_sql.FlavorCosmosDb: nil,
		prom_sql.FlavorMsSql:    {"NVARCHAR(8)", "DATE", "TIME", "DATETIME2", "DATETIMEOFFSET"},
		prom_sql.FlavorMySql:    {"VARCHAR(8)", "DATE", "TIME", "DATETIME", "TIMESTAMP"},
		prom_sql.FlavorOracle:   {"NVARCHAR2(8)", "DATE", "TIMESTAMP(0)", "TIMESTAMP(0)", "TIMESTAMP(0) WITH TIME ZONE"},
		prom_sql.FlavorPgSql:    {"VARCHAR(8)", "DATE", "TIME(0)", "TIMESTAMP(0)", "TIMESTAMP(0) WITH TIME ZONE"},
		prom_sql.FlavorSqlite:   {"VARCHAR(8)", "DATE", "TIME", "DATETIME", "DATETIME"},
	}
	type Row struct {
		id            string
		dataDate      time.Time
	...
	for index, dbtype := range dbtypeList {
		...
		t.Run(dbtype, func(t *testing.T) {
			...
			sql = fmt.Sprintf(sql, tblName)
			for idx, conn := range []*prom_sql.SqlConnect{sqlc, sqlc2} {
				...
				for i, row := range rows {
					...
					}
					{
						...
						f := colNameList[1]
						v, ok := row[f].(time.Time)
						if conn.GetDbFlavor() == prom_sql.FlavorCosmosDb {
							t, err := time.ParseInLocation(time.RFC3339, row[f].(string), sqlc.GetLocation())
							v = _changeLoc(t, sqlc.GetLocation())
							ok = err == nil
						}
					...
					}
					{
						...
						if conn.GetDbFlavor() == prom_sql.FlavorMySql {
