...
func _sqlcVerifyLastCommand(f _testFailedWithMsgFunc, testName string, sqlc *promsql.SqlConnect, cmdName string, cmdCats ...string) {
...
func _testTxProxy_Commit(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
...
func _testTxProxy_DBBegin_Commit(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
...
func _testTxProxy_DBBeginTx_Commit(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
...
func _testTxProxy_ConnBeginTx_Commit(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
...
func _testTxProxy_Rollback(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
...
func _testTxProxy_DBBegin_Rollback(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
...
func _testTxProxy_DBBeginTx_Rollback(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		_testTxProxy_Rollback(testName, dbtype, sqlc, tx, f)
...
func _testTxProxy_ConnBeginTx_Rollback(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		_testTxProxy_Rollback(testName, dbtype, sqlc, tx, f)
...
func _testTxProxy_Prepare(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	if err := sqlInitTable(sqlc, testSqlTableName, false); err != nil {
...
func _testTxProxy_DBBegin_Prepare(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_Prepare(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_Prepare(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_PrepareContext(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	if err := sqlInitTable(sqlc, testSqlTableName, false); err != nil {
		f(fmt.Sprintf("%s failed: error [%s]", testName+"/sqlInitTable/"+dbtype, err))
	}
...
func _testTxProxy_DBBegin_PrepareContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_PrepareContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_PrepareContext(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_Exec(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_Exec(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_Exec(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_Exec(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ExecContext(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_ExecContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_ExecContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_ExecContext(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_Query(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_Query(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_Query(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_Query(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_QueryContext(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_QueryContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_QueryContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_QueryContext(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_QueryRow(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_QueryRow(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_QueryRow(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_QueryRow(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_QueryRowContext(testName, dbtype string, sqlc *promsql.SqlConnect, tx *promsql.TxProxy, f _testFailedWithMsgFunc) {
	type testInfo struct {
		cmd, query string
		params     []interface{}
		metricCat  string
...
func _testTxProxy_DBBegin_QueryRowContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginProxy(); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_DBBeginTx_QueryRowContext(testName, dbtype string, sqlc *promsql.SqlConnect, db *promsql.DBProxy, f _testFailedWithMsgFunc) {
	if tx, err := db.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
...
func _testTxProxy_ConnBeginTx_QueryRowContext(testName, dbtype string, sqlc *promsql.SqlConnect, conn *promsql.ConnProxy, f _testFailedWithMsgFunc) {
	if tx, err := conn.BeginTxProxy(context.TODO(), nil); err != nil {
		f(fmt.Sprintf("%s failed: %s", testName+"/"+dbtype, err))
	} else {
		defer tx.Commit()
