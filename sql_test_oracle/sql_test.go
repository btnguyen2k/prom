...
func newSqlConnectSqlite(driver, url, timezone string, timeoutMs int, poolOptions *promsql.PoolOpts) (*promsql.SqlConnect, error) {
	os.Remove(url)
	sqlc, err := promsql.NewSqlConnectWithFlavor(driver, url, timeoutMs, poolOptions, promsql.FlavorSqlite)
	if err == nil && sqlc != nil {
...
func newSqlConnectMssql(driver, url, timezone string, timeoutMs int, poolOptions *promsql.PoolOpts) (*promsql.SqlConnect, error) {
	sqlc, err := promsql.NewSqlConnectWithFlavor(driver, url, timeoutMs, poolOptions, promsql.FlavorMsSql)
	if err == nil && sqlc != nil {
		loc, _ := time.LoadLocation(timezone)
		sqlc.SetLocation(loc)
	}
...
func newSqlConnectMysql(driver, url, timezone string, timeoutMs int, poolOptions *promsql.PoolOpts) (*promsql.SqlConnect, error) {
	urlTimezone := strings.ReplaceAll(timezone, "/", "%2f")
	url = strings.ReplaceAll(url, "${loc}", urlTimezone)
	url = strings.ReplaceAll(url, "${tz}", urlTimezone)
	url = strings.ReplaceAll(url, "${timezone}", urlTimezone)
	sqlc, err := promsql.NewSqlConnectWithFlavor(driver, url, timeoutMs, poolOptions, promsql.FlavorMySql)
	if err == nil && sqlc != nil {
		loc, _ := time.LoadLocation(timezone)
		sqlc.SetLocation(loc)
	}
...
func newSqlConnectOracle(driver, url, timezone string, timeoutMs int, poolOptions *promsql.PoolOpts) (*promsql.SqlConnect, error) {
	sqlc, err := promsql.NewSqlConnectWithFlavor(driver, url, timeoutMs, poolOptions, promsql.FlavorOracle)
	if err == nil && sqlc != nil {
		loc, _ := time.LoadLocation(timezone)
		sqlc.SetLocation(loc)
	}
...
func newSqlConnectPgsql(driver, url, timezone string, timeoutMs int, poolOptions *promsql.PoolOpts) (*promsql.SqlConnect, error) {
	sqlc, err := promsql.NewSqlConnectWithFlavor(driver, url, timeoutMs, poolOptions, promsql.FlavorPgSql)
	if err == nil && sqlc != nil {
		loc, _ := time.LoadLocation(timezone)
		sqlc.SetLocation(loc)
	}
...
func TestSqlConnect_GetInfo(t *testing.T) {
	...
	driver, dsn, dbFlavor := "sqlite", "./temp/temp.db", promsql.FlavorSqlite
...
func _generatePlaceholders(num int, sqlc *promsql.SqlConnect) string {
	result := ""
	for i := 1; i <= num; i++ {
		switch sqlc.GetDbFlavor() {
		case promsql.FlavorMsSql:
			result += "@p" + strconv.Itoa(i)
		case promsql.FlavorOracle:
			result += ":" + strconv.Itoa(i)
		case promsql.FlavorPgSql, promsql.FlavorCosmosDb:
			result += "$" + strconv.Itoa(i)
		default:
			result += "?"
		}
...
func _dbtypeStr(flavor promsql.DbFlavor) string {
	switch flavor {
...
func sqlInitTable(sqlc *promsql.SqlConnect, table string, insertSampleRows bool) error {
	...
	switch sqlc.GetDbFlavor() {
	case promsql.FlavorSqlite, promsql.FlavorMsSql, promsql.FlavorMySql, promsql.FlavorOracle, promsql.FlavorPgSql:
		for i, n := 1, len(sqlTableColNames); i < n; i++ {
...
var (
	sqlcList   []*promsql.SqlConnect
	sqlc2List  []*promsql.SqlConnect
	dbtypeList []string
)

var _setupTestSqlConnect _testSetupOrTeardownFunc = func(t *testing.T, testName string) {
	sqlcList = make([]*promsql.SqlConnect, 0)
	sqlc2List = make([]*promsql.SqlConnect, 0)
	dbtypeList = make([]string, 0)
	urlMap := sqlGetUrlFromEnv()
	for dbtype, info := range urlMap {
		var sqlc, sqlc2 *promsql.SqlConnect
		var err, err2 error
		switch dbtype {
		case "sqlite", "sqlite3":
